...
function addMissing(target,text,severity){target.push({text:text,severity:severity||'verification'});}
...
if(item.district){if(!p.district)addMissing(missing,'시·군·구 확인','decisive');else if(p.district===item.district)matched.push(item.district+' 거주');else failed.push('시·군·구 불일치');}
...
if(r.requiresIncomeAssetsCheck)addMissing(missing,'소득·재산 기준 확인','decisive');
if(r.exclusionsNeedCheck)addMissing(missing,'세부 제외대상 확인','verification');
if(r.needsPublicTransitUse)addMissing(missing,'대중교통 이용 여부 확인','verification');
var hasDecisiveMissing=missing.some(function(m){return m.severity==='decisive';});
var hasVerificationMissing=missing.some(function(m){return m.severity==='verification';});
var status;
if(failed.length)status='ineligible';
else if(item.automaticEnrollment&&!hasDecisiveMissing&&!hasVerificationMissing)status='automatic';
else if(hasDecisiveMissing)status='needs_info';
else if(hasVerificationMissing)status='likely';
else status='likely';
return{item:item,status:status,matched:matched,failed:failed,missing:missing};}
var labels={automatic:'자동 적용 가능',likely:'받을 가능성이 높아요',needs_info:'추가 정보가 있어야 판단 가능',ineligible:'현재 조건상 제외'};
var classMap={automatic:'high',likely:'high',needs_info:'medium',ineligible:'low'};
function card(x){var i=x.item;var detail='';if(x.matched.length)detail+='<div class="match-reasons"><strong>맞는 조건</strong><ul>'+x.matched.map(function(v){return'<li>✓ '+esc(v)+'</li>';}).join('')+'</ul></div>';if(x.missing.length)detail+='<div class="match-warning">⚠ '+x.missing.map(function(m){return esc(m.text);}).join(' · ')+'</div>';if(x.failed.length)detail+='<div class="match-warning">✕ '+x.failed.map(esc).join(' · ')+'</div>';
...
function run(e){if(e)e.preventDefault();var p=profile();if(p.age==null||!p.region||!p.status||!p.housing){document.getElementById('formError').textContent='거주지역, 출생연도, 경제활동 상태, 주거형태를 입력해 주세요.';return;}document.getElementById('formError').textContent='';var all=rules.map(function(r){return evaluate(r,p);});var automatic=all.filter(function(x){return x.status==='automatic';});var likely=all.filter(function(x){return x.status==='likely';});var needsInfo=all.filter(function(x){return x.status==='needs_info';});var excluded=all.filter(function(x){return x.status==='ineligible';});var shown=automatic.concat(likely,needsInfo);summary.textContent=p.region+(p.district?' '+p.district:'')+' · '+p.birthYear+'년생 · '+p.household+'인 가구 · '+({employed:'직장인',jobseeker:'구직 중',business:'자영업·소상공인',student:'학생',farmer:'농어업',other:'기타'}[p.status]||p.status)+' · '+({jeonse:'전세',rent:'월세',owner:'자가',other:'기타'}[p.housing]||p.housing);count.innerHTML='<strong>'+shown.length+'개</strong> 확인할 혜택 · 제외 '+excluded.length+'개';results.innerHTML=section('🎁 지금 꼭 확인하세요',automatic)+section('👍 받을 가능성이 높아요',likely)+section('📋 추가 정보가 있어야 판단할 수 있어요',needsInfo)+(excluded.length?'<details style="grid-column:1/-1"><summary><strong>현재 조건상 제외된 혜택 '+excluded.length+'개 보기</strong></summary><div class="match-grid" style="margin-top:14px">'+excluded.map(card).join('')+'</div></details>':'');document.getElementById('resultsSection').hidden=false;document.getElementById('resultsSection').scrollIntoView({behavior:'smooth',block:'start'});window.__LAST_MATCH_V2={profile:p,shown:shown,automatic:automatic,likely:likely,needsInfo:needsInfo,excluded:excluded};}
